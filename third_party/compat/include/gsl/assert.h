...
// 2. GSL_THROW_ON_CONTRACT_VIOLATION: a std::fail_fast exception will be thrown
...
#define Expects(cond)  if (!(cond)) \
    throw std::fail_fast("Precondition failure at " __FILE__ ": " GSL_STRINGIFY(__LINE__));
#define Ensures(cond)  if (!(cond)) \
    throw std::fail_fast("Postcondition failure at " __FILE__ ": " GSL_STRINGIFY(__LINE__));
